...
{#each $toast as msg (msg.id)}
  <div
    ...
    class="fixed bottom-6 right-6 z-[100] w-80 max-w-[calc(100vw-2rem)] rounded-lg border shadow-lg {typeConfig[msg.type].bg} {typeConfig[msg.type].border}"
    role="alert"
  >
    <button
      type="button"
    ...
    <div class="flex items-start gap-3 p-4 pr-8">
      <Icon icon={typeConfig[msg.type].icon} class="size-5 shrink-0 {iconClass(msg.type)}" />
      <div class="min-w-0 flex-1">
        <p class="text-sm font-semibold text-foreground">{msg.title}</p>
        {#if msg.description}
          <p class="mt-0.5 text-xs text-muted-foreground">{msg.description}</p>
        {/if}
      </div>
    </div>
  </div>
